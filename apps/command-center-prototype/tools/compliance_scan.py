...
RULES = {
    "contact": [  # 联系方式 = 最硬违规
        ...
        "私我", "私信", "加我", "滴滴我", "dd我", "扣1", "扣一", "联系我", "咨询请", "加好友",
        "主页", "简介", "置顶", "评论区见", "评论区扣", "评论扣", "评论区回复", "评论区call", "看签名", "see profile",
    ],
    "promise": [  # 承诺/夸大结果 = 教育/医美高危触发
        "搞定", "逆袭", "必过", "保过", "包过", "包录取", "包offer", "包你", "100%", "百分百",
        "稳过", "稳拿", "稳进", "必上", "必录", "绝对", "秒过", "速成", "一次过", "轻松搞定", "马上见效",
    ],
    "solicit": [  # 招揽/中介身份口吻 = "我在卖服务"(注意:"名额"是事实词不算招揽,只"名额有限/抢名额"才算)
        "我帮", "帮你", "帮家庭", "帮孩子", "我的学员", "我带的", "咨询我", "找我", "报名",
        "名额有限", "抢名额", "私聊", "付费咨询", "我们机构", "我们团队", "我们工作室", "一对一", "测评领取",
...
def scan(text):
    ...
    for m in re.finditer(r"(?<!\d)1[3-9]\d{9}(?!\d)", text):
        ...
        cats.add("contact")
    # 评论区扣数字引流(扣1/扣个1/扣"1"/扣 1,引号或空格隔开也算)
    for m in re.finditer(r"扣\s*[\"'“”‘’]?\s*\d", text):
        hits.append({"category": "contact", "term": "扣数字引流", "snippet": m.group(0)})
        cats.add("contact")
